...
# --- Configuration ---
# IMPORTANT: Change this to the interface connected to your mirror port
NETWORK_INTERFACE = "eth0" 
FLOW_TIMEOUT = 60  # seconds to wait before a flow is considered inactive
HISTORY_LEN = 100  # Number of recent connections to keep for feature calculation
...
connection_history = deque(maxlen=HISTORY_LEN) # Stores feature dicts of recent connections
...
def process_and_predict(flow_data):
    ...
    # Calculate error rates from history
    ...
    # Fill in other features with approximations or defaults (this is a key simplification)
    # In a real-world system, these would be calculated with more complex state-tracking.
    for col in model_columns:
        if col not in features and col in ['rerror_rate', 'srv_rerror_rate', 'same_srv_rate', 'diff_srv_rate', 'srv_diff_host_rate', 
            ...
            features[col] = 0.0
        elif col not in features: # Default for content-based features
            features[col] = 0
...
def packet_callback(packet):
    ...
    if flow_id not in active_flows:
        active_flows[flow_id] = {
            ...
            'flags': set(), 'wrong_fragment': 0, 'urgent': 0,
    ...
    # Check for FIN or RST to close flow immediately
...
if __name__ == '__main__':
    ...
    # Start the flow timeout manager in a separate thread
    manager_thread = Thread(target=flow_manager, daemon=True)
    manager_thread.start()

    try:
        # Start sniffing. Run with sudo.
